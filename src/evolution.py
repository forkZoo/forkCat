...
from src.genetics import CatDNA, GeneticsEngine, TraitCategory
...
class EvolutionAgent:
    """AI agent that evolves cats intelligently"""
    ...
    def evolve_with_ai(self, dna: CatDNA, days_passed: int = 1) -> CatDNA:
        """
        Use AI to intelligently evolve the cat
        """
    ...
    def _create_evolution_prompt(self, traits: dict, days: int, generation: int) -> str:
        ...
        return f"""You are an AI evolution agent for ForkCat - a digital pet that lives on GitHub.

Your task is to evolve this cat's appearance in a subtle, aesthetically pleasing way.
...
{{
  ...
  "evolution_story": "Your cat is maturing, developing a golden sheen..."
}}

Keep changes minimal (0-2 traits). Consider the cat's current aesthetic."""
    ...
    def _apply_evolution(self, dna: CatDNA, decision: dict) -> CatDNA:
    ...
    def generate_evolution_story(self, old_dna: CatDNA, new_dna: CatDNA) -> str:
        ...
        if not changes:
            return "Your cat rested today. No visible changes."
        
        prompt = f"""Generate a short, whimsical story (2-3 sentences) about a cat's evolution.
...
Make it fun and engaging, like a Tamagotchi update message."""
        ...
        try:
            ...
            return f"Your cat evolved! Changes: {', '.join(changes)}"
...
def main():
    ...
    print("🧬 ForkCat Evolution Agent Test\n")
