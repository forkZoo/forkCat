...
Command-line interface for managing your cat.
"""
...
from src.genetics import GeneticsEngine, CatDNA
from src.storage import CatStorage
from src.visualizer import CatVisualizer
...
@click.group()
def cli():
    """🐵 ForkCat - Your AI-powered digital pet on GitHub"""
    pass
...
def init(from_fork, force):
    ...
    console.print("\n🐵 [bold cyan]Initializing ForkCat...[/bold cyan]\n")
    ...
    if existing_dna:
        console.print("[yellow]⚠️  Cat already exists![/yellow]")
    ...
    if from_fork:
        ...
        if not dna:
            ...
            console.print("[cyan]   Generating new cat instead...[/cyan]")
            dna = GeneticsEngine.generate_random_dna()
    else:
        console.print("[cyan]🎲 Generating random cat...[/cyan]")
        dna = GeneticsEngine.generate_random_dna()
    
    # Save DNA
    storage.save_dna_locally(dna)
    storage.save_stats(dna, age_days=0)
    storage.save_history_entry(dna, "🎉 Your cat was born!")
    
    # Generate initial visualization
    svg = CatVisualizer.generate_svg(dna)
    svg_file = Path("cat_data/cat.svg")
    svg_file.write_text(svg)
    
    # Archive with timestamp
    from datetime import datetime
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
    archive_file = Path(f"cat_evolution/{timestamp}_cat.svg")
    archive_file.parent.mkdir(exist_ok=True)
    archive_file.write_text(svg)
    
    # Display info
    console.print("\n[bold green]✅ Cat initialized![/bold green]\n")
    
    table = Table(title="Your Cat")
...
def evolve(ai, strength):
    """Evolve your cat"""
    console.print("\n🧬 [bold cyan]Evolving cat...[/bold cyan]\n")
    ...
    if not dna:
        console.print("[red]❌ No cat found! Run 'init' first.[/red]")
        return
    ...
    if ai:
        ...
        try:
            ...
            story = "Your cat evolved randomly!"
    else:
    ...
    svg = CatVisualizer.generate_svg(evolved_dna)
    svg_file = Path("cat_data/cat.svg")
    svg_file.write_text(svg)
...
@cli.command()
def show():
    """Show current cat stats"""
    console.print("\n🐵 [bold cyan]Your Cat[/bold cyan]\n")
...
@cli.command()
def visualize():
    """Generate and save cat visualization"""
...
@cli.command()
def update_readme():
    """Update README with current cat"""
    ...
    svg = CatVisualizer.generate_svg(dna, width=400, height=400)
    svg_file = Path("cat_data/cat.svg")
    svg_file.write_text(svg)
    ...
    # Update cat display section with image reference
    cat_section = '''<!-- CAT_DISPLAY_START -->
<div align="center">
...
<!-- CAT_DISPLAY_END -->'''
    ...
    # Replace section
    import re
    pattern = r'<!-- CAT_DISPLAY_START -->.*?<!-- CAT_DISPLAY_END -->'
    readme = re.sub(pattern, cat_section, readme, flags=re.DOTALL)
    
    # Update stats section
    history = storage.get_history()
    age_days = len(history)
    
    stats_section = f'''<!-- CAT_STATS_START -->
...
<!-- CAT_STATS_END -->'''
    ...
    pattern = r'<!-- CAT_STATS_START -->.*?<!-- CAT_STATS_END -->'
